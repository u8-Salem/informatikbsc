...
typedef struct {
    ...
    Position values[POSITION_STACK_SIZE]; // valid values are at 0..length-1
} PositionStack;

PositionStack make_position_stack() {
    return (PositionStack){.length = 0};
}

void push(PositionStack *s, Position p) {
    require("stack not full", s->length < POSITION_STACK_SIZE);
    s->values[s->length++] = p;
}

Position pop(PositionStack *s) {
    require("stack not empty", s->length > 0);
    return s->values[--s->length];
}

Position random_position(PositionStack *s) {
    require("stack not empty", s->length > 0);
...
// Finds the positions of all valid moves.
PositionStack valid_moves(Game* g, Player* p) {
    PositionStack s = make_position_stack();
    for (int y = 0; y < N; y++) {
        for (int x = 0; x < N; x++) {
            Position pos = {x, y};
            if (legal_move(g, p, pos)) {
...
bool random_valid_move(Game* g, Player* p, /*out*/Position* pos) {
    // clock_t t = clock(); // get current timestamp
    PositionStack s = valid_moves(g, p);
    if (s.length <= 0) {
        return false;
    }
    *pos = random_position(&s);
